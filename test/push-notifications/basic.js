...
describe("push-notifications: instantiation and class properties", () => {
  ...
  describe("check getPushMethodByRegId returns expected push method types", () => {
    const regIds = {
      ...
      iOSRegId: "43e798c31a282d129a34d84472bbdd7632562ff0732b58a85a27c5d9fdf59b69",
      iOSRegIdLong:
    ...
    it("Android / FCM", () => {
      ...
      expect(pn.getPushMethodByRegId(regIds.iOSRegIdLong).regId).to.equal(regIds.iOSRegIdLong);
      expect(pn.getPushMethodByRegId(regIds.iOSRegIdLong).pushMethod).to.equal(FCM_METHOD);
    });

    it("iOS / APN", () => {
      const pn = new PN();
      expect(pn.getPushMethodByRegId(regIds.iOSRegId).regId).to.equal(regIds.iOSRegId);
      expect(pn.getPushMethodByRegId(regIds.iOSRegId).pushMethod).to.equal(APN_METHOD);

      expect(pn.getPushMethodByRegId(regIds.iosObject).regId).to.equal(regIds.iosObject.id);
      expect(pn.getPushMethodByRegId(regIds.iosObject).pushMethod).to.equal(APN_METHOD);

      expect(pn.getPushMethodByRegId(regIds.iosObjectWhatever).regId).to.equal(
        regIds.iosObjectWhatever.id
      );
      expect(pn.getPushMethodByRegId(regIds.iosObjectWhatever).pushMethod).to.equal(APN_METHOD);
    });

    it("iOS / APN long", () => {
      const pn = new PN();
      expect(pn.getPushMethodByRegId(regIds.iOSRegIdLong).regId).to.equal(regIds.iOSRegIdLong);
      expect(pn.getPushMethodByRegId(regIds.iOSRegIdLong).pushMethod).to.equal(APN_METHOD);
    });
    ...
    it("Windows Phone / WNS", () => {
      ...
      expect(pn.getPushMethodByRegId(regIds.windowsPhoneRegId).pushMethod).to.equal(WNS_METHOD);
    });

    it("Amazon / ADM", () => {
      const pn = new PN();
      expect(pn.getPushMethodByRegId(regIds.amazonRegId).regId).to.equal(regIds.amazonRegId);
      expect(pn.getPushMethodByRegId(regIds.amazonRegId).pushMethod).to.equal(ADM_METHOD);
    });

    it("Web / WEB", () => {
      const pn = new PN();
      expect(pn.getPushMethodByRegId(regIds.webRegId).regId).to.equal(regIds.webRegId);
      expect(pn.getPushMethodByRegId(regIds.webRegId).pushMethod).to.equal(WEB_METHOD);
    });

    it("Unknown / UNKNOWN", () => {
      const pn = new PN();
      expect(pn.getPushMethodByRegId(regIds.unknownRegId).regId).to.equal(regIds.unknownRegId);
      expect(pn.getPushMethodByRegId(regIds.unknownRegId).pushMethod).to.equal(UNKNOWN_METHOD);
    });
  });
});
